...
<div id="right">
	...
	<div id="content">
		...
		<form id="set_frm" action="set_algrcom.php" method="get">
		...
		<div class="main_contitle">
			<img src="../images/title_06_09.png" alt="경보그룹 구성">
		</div>
		<div class="right_bg">
		<ul class="set_ulwrap_nh b0 w80">
			<li class="tb_alarm_lm">
				<div class="alarm">
					<ul>
						<li class="alarm_gry h50p">경보그룹</li>
						<li class="li100_nor p0 min500 max500">
							...
							?>
									<tr id="list_<?=$val['ALARM_GRP_NO']?>">
										<td id="l_ALARM_GRP_NO" class="li30 hi25"><?=$val['ALARM_GRP_NO']?></td>
										<td class="li70 hi25"><?=$val['ALARM_GRP_NAME']?></td>
									</tr>
							<? 
								}
							}
...
$(document).ready(function(){
	...
	// 그룹 구성 추가
	function group_in(){
		var param = "mode=algr_com_in&"+$("#set_frm").serialize();
		$.ajax({
	        type: "POST",
	        ...
	        success : function(data){
		        if(data.result){
					group_load( "list_"+$("#ALARM_GRP_NO").val() ); // 그룹 구성 호출
		        }else{
	...
	// 그룹 구성 삭제
	function group_de(){
		var param = "mode=algr_com_de&"+$("#set_frm").serialize();
		$.ajax({
	        type: "POST",
	        url: "../_info/json/_set_json.php",
		    data: param,
	...
	function group_load(id){
		var l_ALARM_GRP_NO = $("#"+id+" #l_ALARM_GRP_NO").text();
		$("#ALARM_GRP_NO").val(l_ALARM_GRP_NO);
		
        $("#list_table2 tbody").empty(); // 소속 장비 리스트 초기화
		var param = "mode=algr_comin_view&ALARM_GRP_NO="+l_ALARM_GRP_NO;
		$.ajax({
	        type: "POST",
	        url: "../_info/json/_set_json.php",
		    data: param,
		...
		var param = "mode=algr_comde_view&ALARM_GRP_NO="+l_ALARM_GRP_NO;
		$.ajax({
	        type: "POST",
	        url: "../_info/json/_set_json.php",
		    data: param,
