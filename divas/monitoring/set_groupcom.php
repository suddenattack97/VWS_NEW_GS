						...
						<li class="li100_nor p0 min500 max500">
							...
							?>
									<tr id="list_<?=$val['GROUP_ID']?>">
										...
										<td id="l_ORGAN_ID" style="display:none"><?=$val['ORGAN_ID']?></td>
									</tr>
							<? 
								}
							}
...
$(document).ready(function(){
	...
	// 그룹 구성 추가
	function group_in(){
		var param = "mode=group_com_in&"+$("#set_frm").serialize();
		$.ajax({
	        type: "POST",
	...
	// 그룹 구성 삭제
	function group_de(){
		var param = "mode=group_com_de&"+$("#set_frm").serialize();
		$.ajax({
	        type: "POST",
	        url: "../_info/json/_set_json.php",
		    data: param,
	...
	function group_load(id){
		...
		var l_ORGAN_ID = $("#"+id+" #l_ORGAN_ID").text();
		$("#GROUP_ID").val(l_GROUP_ID);
		$("#ORGAN_ID").val(l_ORGAN_ID);

        $("#list_table2 tbody").empty(); // 소속 장비 리스트 초기화
		var param = "mode=group_comin_view&GROUP_ID="+l_GROUP_ID;
		$.ajax({
	        type: "POST",
	        url: "../_info/json/_set_json.php",
		    data: param,
		...
		var param = "mode=group_comde_view&GROUP_ID="+l_GROUP_ID;
		$.ajax({
	        type: "POST",
	        url: "../_info/json/_set_json.php",
		    data: param,
