...
class Spreadsheet_Excel_Reader {
	...
	function dump($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		$out = "<table id=\"sheet_$sheet\" class=\"$table_class\" cellspacing=\"0\">";
		if ($col_letters) {
			$out .= "<thead>\n\t<tr>";
			if ($row_numbers) {
				$out .= "\n\t\t<th>&nbsp</th>";
		...
		for($row=1;$row<=$this->rowcount($sheet);$row++) {
			...
			for($col=1;$col<=$this->colcount($sheet);$col++) {
				...
				if(!$this->sheets[$sheet]['cellsInfo'][$row][$col]['dontprint']) {
					...
					$out .= "\n\t\t<td id=\"col_$col\" style=\"$style\"" . ($colspan > 1?" colspan=$colspan":"") . ($rowspan > 1?" rowspan=$rowspan":"") . ">";
