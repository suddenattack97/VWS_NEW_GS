    ...
    case 'displace':
    	...
    	if($arr_area_code){
    		foreach($arr_area_code as $key => $val){
		    	...
		    	// $qry = " SELECT displace, displace_date FROM displace_hist
				// 		 WHERE area_code = '".$val."'
				// 		 AND data_type = 'M' ";
		    	// if(TEST == "0"){
		    	// 	$qry.= " AND displace_date BETWEEN {TS '".$DM->getMinDisTime()."'} AND {TS '".$DM->getMinTime()."'} ";
		    	// }else{
		    	// 	$qry.= " AND displace_date = '".TEST_DATE."' ";
		    	// }
				// $qry.= " ORDER BY displace_date DESC LIMIT 2 ";
				
				$rtu_id = $arr_rtu[$val]['rtu_id'];

				$qry = " SELECT event_date, event_code FROM event_hist
				WHERE rtu_id = '".$val."'
				AND data_type = 'M' ";
				if(TEST == "0"){
					$qry.= " AND displace_date BETWEEN {TS '".$DM->getMinDisTime()."'} AND {TS '".$DM->getMinTime()."'} ";
				...
				$arr_data[$key]['displace'] = "-";
				$arr_data[$key]['displace_date'] = "-";
				$arr_data[$key]['day'] = "-";
				$arr_data[$key]['date'] = "-";
				$arr_data[$key]['state'] = false;

		    	if( isset($data[0]['displace_date']) ){
					if( $data[0]['displace_date'] < $DM->getMinDisTime()){
					}else{
					$arr_data[$key]['day'] = $data[0]['displace']*$calc;
		    		$arr_data[$key]['date'] = $data[0]['displace_date'];
					}

					if($data[0]['displace'] < $data[1]['displace'] || $data[0]['displace'] > $data[1]['displace']){
