...
Class DisplaceInfo {
	...
	/* 생성자 */
	function DisplaceInfo($DB,$DateMake,$rfutil=null) {
		$this->DB = $DB;
		$this->DM = $DateMake;
		$this->rfutil = $rfutil;
	}
...
//exit;
	...
	function getTimeListValue($area_code, $type, $startdate, $enddate) {
		$sql = " SELECT IFNULL(b.displacement_date, '-') AS displacement_date, IFNULL(b.displacement_curr, '-') AS rain, a.num 
				 FROM statistics_tmp AS a
				 LEFT JOIN (
				 SELECT * FROM displacement_hist
				 WHERE area_code = '" . $area_code . "' AND data_type = '".$type."' AND displacement_date BETWEEN {ts '".$startdate."'} AND {ts '".$enddate."'}
				 ) AS b ON a.num = DATE_FORMAT(b.displacement_date, '%k')
				 WHERE a.type = 'H'
				 ORDER BY a.num ASC ";
		$rs = $this->DB->execute($sql);
		...
		for($i=0; $i<$this->DB->NUM_ROW(); $i++){
			$this->TimeListValue[$i]     = ($rs[$i]['rain'] != "-") ? $rs[$i]['rain'] / 10000 : "-";
