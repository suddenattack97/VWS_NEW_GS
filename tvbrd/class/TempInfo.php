...
Class TempInfo {
  ...
  public $TimeListDateValue; // 시간 단위 날짜
	...
	function getTimeListValue($area_code, $type, $startdate, $enddate) {
		$sql = " SELECT IFNULL(b.temp_date, '-') AS temp_date, IFNULL(b.avr_val, '-') AS avr_val, 
				...
				join (select num from statistics_tmp where type = 'M') as t2	
				 ...
				 AND temp_date BETWEEN {ts '".$startdate."'} AND {ts '".$enddate."'}
				 order by temp_date desc
				 ) AS b 
				 ON t1.num = DATE_FORMAT(b.temp_date, '%k') 
				 AND t2.num = DATE_FORMAT(b.temp_date, '%i') 
				 group by numC
				 ORDER BY b.temp_date DESC ";
		$rs = $this->DB->execute($sql);
