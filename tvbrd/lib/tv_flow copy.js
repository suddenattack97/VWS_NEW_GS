function flow(kind, arr_area_code){ // 수위 - 수위만 있는 장비
	// 객체 생성
	if(kind == 1){
		if(arr_area_code){
			$.each(arr_area_code, function(index, item){
				...
				var box_content = '\n\
					<div id="flow_'+item+'" class="label water">\n\
						<ul>\n\
				...
				if(map_kind == 2){
					...
					$("#flow_"+item).css("height", "");
								...
								// 이벤트
								$(document).on("click", "#flow_"+item, function(){
									if(map_kind == 1){
										if($("input:checkbox[id='overlay_move']").is(":checked")){
											...
											$("#flow_"+item).css('box-shadow','rgb(255 0 0 / 70%) 0px 0px 10px 3px');
												...
												var point = new Array(e.coord.lat()+zoom_lat , e.coord.lng());
													swal({
														...
														if(isConfirm){
															...
															$("#flow_"+item).css('box-shadow','');
															arr_rtu[item]['polyline'].setStyles('strokeColor','#303A94');
				...
				$(document).on("mouseover", "#flow_"+item, function(){
		...
		if( (map_kind == 1 || map_kind == 2) && jQuery.inArray("2", map_data) != "-1" ){
			...
			if(arr_area_code){
				$.each(arr_area_code, function(index, item){
					...
					if(map_kind == 1){
						if(arr_rtu[item]['marker'].getIcon().url != "img/icon_s_02.png"){
							arr_rtu[item]['marker'].setIcon(
								{
									url: "img/icon_s_02.png",
									size: new naver.maps.Size(21, 36)
						...
						if(arr_rtu[item]['state']){
							if(arr_rtu[item]['marker'].getIcon().url != "img/icon_s_02_g.png"){
								arr_rtu[item]['marker'].setIcon(
									{
										url: "img/icon_s_02_g.png",
										size: new naver.maps.Size(21, 36)
							...
							if(arr_rtu[item]['marker'].getIcon().url != "img/icon_s_02_o.png"){
								arr_rtu[item]['marker'].setIcon(
									{
										url: "img/icon_s_02_o.png",
										size: new naver.maps.Size(21, 36)
									}
								);
							}
						...
						if(arr_rtu[item]['line']){
							arr_rtu[item]['polyline'].setOptions({ "strokeColor" : "#303A94" });
						}else{
				...
				if(map_kind != 2){
					$.post( "controll/flow.php", { "mode" : "flow", "arr_area_code" : arr_area_code }, function(response){
						$.each(response.list, function(index, item){
							$("#flow_"+item.area_code+" .label_dat").html(item.day);
						});
					}, "json");
				}
			}
