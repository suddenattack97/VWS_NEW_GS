var global_flow_area_code; //이벤트가 등록된 수위 장비 
function flow(kind, arr_area_code){ // 강우 - 강우만 있는 장비
	// 객체 생성
	global_flow_area_code = arr_area_code;
	if(kind == 1){
		if(arr_area_code){
			$.each(arr_area_code, function(index, item){
				...
				if(!document.getElementById("flow_"+item)){
					var box_content= document.createElement("div"); // 오버레이 팝업설정 
					box_content.innerHTML+="<div id='flow_"+item+"' class='label water'><ul><li class='label_top'>"+tmp_name+"</li><li class='label_dat'>&nbsp;</li></ul></div><div class='label_bot'><img src='img/label_bot.png'></div>";
				}
				if(!document.getElementById("flow_"+item+"_marker")){
					var marker_content= document.createElement("div"); // 오버레이 팝업설정 
					marker_content.innerHTML+="<div id='flow_"+item+"_marker' style='margin-top:-38px; margin-left: -10px;' class=''><img style='cursor:"+(map_control_type == 1 ? 'not-allowed' : 'pointer')+"; -webkit-user-drag: none;' src='img/icon_s_02.png'/></div>";
				}
				...
				$("#flow_"+item).parent().parent().addClass('overlay');
				$("#flow_"+item+"_marker").parent().parent().addClass('marker');
				...
				var marker_el = document.getElementById('flow_'+item+'_marker');
				...
				if( Number(map_level) < Number(over_level) ){
					if(arr_rtu[item]['overlay_on']){
						$("#flow_"+item).show();
						$("#flow_"+item).next().show();
    	    		}else{
						$("#flow_"+item).hide();
						$("#flow_"+item).next().hide();
    	    		}
				}
				...
				if(map_kind == 2){
					...
					$("#flow_"+item).css("height", "28px");
				}else{
					$("#flow_"+item).css("height", "");
				}

				// 이벤트
				$("#flow_"+item).on("click" , function(){
					if(map_kind == 1){
						slide_on("flow", item);
					}else if(map_kind == 2){
				...
				map.on('pointermove', function(evt) {
					if (arr_rtu[item]['marker'].get('dragging') === true) {
						...
						$("#flow_"+item).css('margin-top','20px');
						$("#flow_"+item+"_marker").css('margin-top','-20px');
						$("#flow_"+item+"_marker").css('margin-left','-10px');
					}
				});
			...
			if(arr_area_code){
				$.each(arr_area_code, function(index, item){
					...
					if( Number(map_level) < Number(over_level) ){
						...
						// $("#flow_"+item).parent().parent().removeClass('overlay_down');
						// $("#flow_"+item).parent().parent().addClass('overlay_up');
						$("#flow_"+item).parent().parent().css('z-index', 120);
						$("#flow_"+item+"_marker").parent().parent().css('z-index', 111);
		    		}
				...
				if(map_kind != 2){
					$.post( "controll/flow.php", { "mode" : "flow", "arr_area_code" : arr_area_code }, function(response){
						$.each(response.list, function(index, item){
							$("#flow_"+item.area_code+" .label_dat").text(item.day);
							var tmp_emd_cd = arr_rtu[item.area_code]['emd_cd'];
							arr_rtu[item.area_code]['flow'] = item.day;
						});
					}, "json");
				}
			}
...
function flow_event(area_code,state,level) {
	if(state == 1){
		// circle[area_code].setVisible(true);
		if(level == 1){
			if(!$("#wave_"+area_code).hasClass('waves')){
				$("#flow_"+area_code).parent().parent().css('z-index','121');
				$("#flow_"+area_code).show();
				$("#flow_"+area_code).next().show();
				$("#flow_"+area_code+"_marker").show();
				$("#flow_"+area_code+"_marker").parent().parent().css('z-index','112');

				if($("#flow_"+area_code).css('display') == 'block'){
			...
			if(!$("#wave_"+area_code).hasClass('waves2')){
				$("#flow_"+area_code).parent().parent().css('z-index','122');
				$("#flow_"+area_code).show();
				$("#flow_"+area_code).next().show();
				$("#flow_"+area_code+"_marker").show();
				$("#flow_"+area_code+"_marker").parent().parent().css('z-index','113');
		...
		$.post( "controll/flow.php", { "mode" : "flow", "arr_area_code" : global_flow_area_code }, function(response){
			$.each(response.list, function(index, item){
				$("#flow_"+item.area_code+" .label_dat").text(item.day);
				var tmp_emd_cd = arr_rtu[item.area_code]['emd_cd'];
				arr_rtu[item.area_code]['flow'] = item.day;
		...
		if($("#flow_"+area_code).css('display') != 'none'){
			$("#wave_"+area_code).removeClass('waves');
			$("#wave_"+area_code).removeClass('waves2');
			$("#wave_"+area_code).hide();
			$("#flow_"+area_code).parent().parent().css('z-index', 110);
			$("#flow_"+area_code+"_marker").parent().parent().css('z-index', 105);
			// $("#flow_"+area_code).hide();
			// $("#flow_"+area_code).next().hide();
			// $("#flow_"+area_code+"_marker").hide();
		}
	}
}
